...
function wpmm_do_display_map_single( $attr ) {
	global $wp_query;
	...
	$stores = json_encode( wpmm_fetch_stores_single( $term_slug,$current_id ) );
	...
	if ( 'null' != $stores ) {
		...
		$lat = get_post_meta( $current_id, '_wpmm_latitude', true );
		$lng = get_post_meta( $current_id, '_wpmm_longitude', true );
